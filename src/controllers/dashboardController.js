...
exports.getStats = async (req, res, next) => {
  try {
    ...
    const [
      ...
      Invoice.sum('paid_amount', { where: { status: 'paid' } }),
      Invoice.sum('paid_amount', { where: { status: 'paid', payment_date: { [Op.gte]: startOfMonth } } }),
      Invoice.sum('paid_amount', { where: { status: 'paid', payment_date: { [Op.between]: [startOfLastMonth, endOfLastMonth] } } }),
